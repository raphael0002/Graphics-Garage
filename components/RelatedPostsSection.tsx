...
export const RelatedPostsSection = ({ currentPost }: RelatedPostsSectionProps) => {
    ...
    useEffect(() => {
        const fetchRelatedPosts = async () => {
            try {
                const response = await fetch(`/api/blog/posts?category=${currentPost.category}&limit=3`)
                const data = await response.json()

                // Filter out the current post and limit to 3
                const filtered = data.posts.filter((post: BlogPostData) => post._id !== currentPost._id).slice(0, 3)
            ...
            <div className="container mx-auto px-4 md:px-6">
                <div className="max-w-4xl mx-auto">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                        <h2 className="text-3xl font-bold dark:text-white text-gray-900 mb-8">Related Posts</h2>
                                ...
                                >
                                    <Link href={`/blog/${post.slug}`}>
                                        ...
                                        <div className="p-6">
                                            ...
                                            <p className="text-gray-600 dark:text-gray-100 text-sm mb-4 line-clamp-2">{post.excerpt}</p>
